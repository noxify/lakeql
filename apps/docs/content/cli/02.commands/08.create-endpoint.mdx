...
| `partitioning`       | boolean / string | No       | Controls file partitioning. `true` (default) partitions by write timestamp, `false` disables partitioning, or a field name string to partition by that field. Only applies to `full_load_append` and `append`. |
