...
| `partitioning`       | boolean / string | No       | Controls file partitioning. `true` (default) partitions by write timestamp, `false` disables partitioning, a field name string to partition by that field, or a custom format string (e.g. `"customer_id/event_date:year"`) for multi-field partitioning. Only applies to `full_load_append` and `append`. |
