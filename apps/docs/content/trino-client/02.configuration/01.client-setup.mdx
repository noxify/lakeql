...
## TrinoClientProps Interface

```typescript
interface TrinoClientProps {
  host: string
  port: number
  auth: Auth
  catalog: string
...
| --------- | -------- | ---------------------------------------------------------------------------------- |
| `host`    | Yes      | Trino coordinator hostname including protocol (e.g. `"https://trino.example.com"`) |
| `port`    | Yes      | Port the coordinator listens on (commonly `8080` or `8443`)                        |
| `auth`    | Yes      | Authentication credentials — basic or bearer                                       |
| `catalog` | Yes      | Default catalog for all queries                                                    |
| `schema`  | No       | Default schema for all queries                                                     |
| `source`  | No       | Identifies the client in Trino logs (defaults to `"nodejs"`)                       |
