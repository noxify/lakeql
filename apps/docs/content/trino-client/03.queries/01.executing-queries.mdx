...
```typescript
async query<T>({ sql, impersonateAs?, gotOpts? }): Promise<T[]>
```

| Parameter       | Type      | Description                                                         |
| --------------- | --------- | ------------------------------------------------------------------- |
| `sql`           | `string`  | The SQL statement to execute                                        |
| `impersonateAs` | `string?` | Optional Trino user to impersonate via `X-Trino-User`               |
| `gotOpts`       | `object?` | Optional [got](https://github.com/sindresorhus/got) request options |

Returns a promise that resolves to a flat array of all rows from every result page.
