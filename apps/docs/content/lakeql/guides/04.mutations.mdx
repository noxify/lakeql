...
<Note>
  The Endpoint Builder adds the `load_timestamp` field automatically when
  timestamp partitioning is active. Fields with `readOnly: true` are queryable
  via GraphQL but cannot be provided as mutation input.
</Note>
