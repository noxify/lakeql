...
- **`true`** (default) — Partitions by the write timestamp. A `load_timestamp` field (DateTime, readOnly) is added to the endpoint definition and populated automatically at runtime.
