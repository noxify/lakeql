...
Once `create-lakeql-app` finishes, follow these steps to get your API running.
...
```bash
pnpm cli pull --catalog hive --schema myschema
```
...
- `src/schemas/generated/hive/myschema/<table>/query-schema.ts` — GraphQL schema
...
```bash
pnpm cli pull --catalog hive --schema sales --table orders --table customers --table products
```
...
```bash
pnpm cli pull --catalog hive --schema new_schema
```
