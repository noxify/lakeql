...
Create a new file in your schema path directory:

```typescript path="src/schemas/custom/analytics-query-schema.ts"
...
The API server loads all schema files from the configured `schemaPath` directory. Any file ending in `query-schema.ts` (or placed in the schema directory) that imports and uses `builder` will be included in the final GraphQL schema.

```typescript
// In your defineConfig, specify where schemas live
import { defineConfig } from "@lakeql/api"
import { allConfigs } from "./config-registry"

export default defineConfig({
  allConfigs,
  schemaPath: "./src/schemas", // All .ts files here are loaded
})
```
...
```typescript path="src/schemas/custom/health-query-schema.ts"
import { builder } from "@lakeql/api"
...
- **Separate files** — Keep custom resolvers in their own files so `lakeql-cli pull` won't overwrite them
- **Name carefully** — Prefix custom types to avoid conflicts (e.g. `CustomAnalyticsResult` vs generated `OrdersResult`)
- **Use the builder** — Always import `builder` from `@lakeql/api` to ensure types are registered in the same schema
- **Check permissions** — Call `context.hasReadPermission` or implement your own auth checks in custom resolvers
