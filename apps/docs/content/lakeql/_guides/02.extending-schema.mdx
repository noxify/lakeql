...
<Note>
  ...
  `src/schemas/custom/`) so the API server loads them at startup. Never place
  custom code inside `schemas/generated/` — it will be overwritten on the next
  `pull`.
</Note>
...
- **Place in `schemas/custom/`** — Keep custom code separate from `schemas/generated/` so it's never overwritten
