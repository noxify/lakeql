...
| `partitioning`       | boolean / string | No       | Controls file partitioning. `true` (default) partitions by write timestamp, `false` disables partitioning (flat layout), or a field name string to partition by that field. Ignored for `full_load`. |
| `partitioningFormat` | string           | No       | Partition granularity. One of `"year"`, `"year/month"`, or `"year/month/day"` (default). Only applies when `partitioning` is not `false`.                                                            |
