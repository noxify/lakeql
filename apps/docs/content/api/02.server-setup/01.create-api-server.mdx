...
| Field    | Description                                           |
| -------- | ----------------------------------------------------- |
| `app`    | The Hono application instance with all routes mounted |
| `logger` | Configured logger instance from `@lakeql/logger`      |
| `yoga`   | The GraphQL Yoga server handling `/graphql` requests  |
...
If you don't need access to the `app` instance, use `startApiServer` which calls `createApiServer` internally and binds to a port:
