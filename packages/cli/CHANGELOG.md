...
- f5fb9de: Show compact live per-item progress for bulk pull entries with more than 10 tables or views, cap total bulk pull concurrency at 8 operations by default, and add a `--concurrency` option so both bulk and non-bulk multi-item pulls can tune that limit.
...
- 2acd5ad: Fix config loading when using `lakeql.config.json` on Node.js 24 with ESM.
  ...
  When `c12` fails with `ERR_IMPORT_ATTRIBUTE_MISSING` for JSON config imports, `@lakeql/cli` now falls back to reading `lakeql.config.json` directly and still applies default config values.

  The CLI also exits successfully when invoked without arguments, printing the top-level help instead of treating the missing command as an error.
