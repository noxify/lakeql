...
import { buildInitCommandStructure } from "./init-metadata"
...
export default function initCommand() {
  const program = buildInitCommandStructure().action(async () => {
    ...
    if (existing) {
      const overwrite = await select(`${existing} already exists. Overwrite?`, {
        choices: [
          { label: "Yes", value: "yes" },
          { label: "No", value: "no" },
        ],
